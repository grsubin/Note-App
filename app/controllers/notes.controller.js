...
const updateNoteById = async (req, res, next) => {
  try {
    ...
    const dbNote = (
      await pool.query(
        "SELECT title, note_body FROM note WHERE user_id = $1 AND id = $2 AND deleted_at IS NULL ",
        [userId, noteId]
      )
    ).rows[0];
    ...
    const updatedNote = (
      await pool.query(
        "UPDATE note SET updated_at = NOW(), title = $1, note_body = $2 WHERE user_id = $3 AND id =$4 RETURNING *",
        [dbNote.title, dbNote.note_body, userId, noteId]
      )
    ).rows[0];
